...
ForecastCarouselWidget::ForecastCarouselWidget(ForecastModel *model, uint16_t y)
    : Widget(0, y, 240, 67), model(model), carousel(&tft, 0, y, 240, 67) {
...
bool ForecastCarouselWidget::handleTouchOff(uint16_t x, uint16_t y,
                                            uint16_t z) {
...
void ForecastCarouselWidget::drawSingleDay(int32_t x, int32_t y,
                                           uint8_t dayIndex) {
  if (isVisible) {
    ...
    sprite.loadFont(ArialBold14);
    sprite.setTextDatum(BC_DATUM);
    sprite.setTextColor(TFT_ORANGE, TFT_BLACK);
    sprite.setTextPadding(width / 3);

    struct tm *timeNow = model->getForecastTime(dayIndex);

    sprite.drawString(String(FPSTR(WEEK_DAYS_NAMES[timeNow->tm_wday])),
                      width / 6, 13);
    ...
    drawBmpFromFile(model->getForecastIconFileName(dayIndex), 15, 13 + 13,
                    &sprite);
...
void ForecastCarouselWidget::drawForecast1(TFT_eSPI *tft, CarouselState *state,
                                           int16_t x, int16_t y) {
  ForecastCarouselWidget *instance =
      static_cast<ForecastCarouselWidget *>(state->userData);
  if (instance->isVisible &&
      ((state->frameState == FIXED && instance->lastFrameState != FIXED) ||
       (state->frameState == IN_TRANSITION && (x != 0 || y != 0)))) {
    instance->drawSingleDay(0 * instance->width / 3 + x, instance->y + y, 1);
    instance->drawSingleDay(1 * instance->width / 3 + x, instance->y + y, 2);
...
void ForecastCarouselWidget::drawForecast2(TFT_eSPI *tft, CarouselState *state,
                                           int16_t x, int16_t y) {
  ForecastCarouselWidget *instance =
      static_cast<ForecastCarouselWidget *>(state->userData);
  if (instance->isVisible &&
      ((state->frameState == FIXED && instance->lastFrameState != FIXED) ||
       (state->frameState == IN_TRANSITION && (x != 0 || y != 0)))) {
    instance->drawSingleDay(0 * instance->width / 3 + x, instance->y + y, 4);
    instance->drawSingleDay(1 * instance->width / 3 + x, instance->y + y, 5);
    instance->drawSingleDay(2 * instance->width / 3 + x, instance->y + y, 6);
