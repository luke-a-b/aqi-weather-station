...
ClockSmallWidget::ClockSmallWidget(
    ClockModel *model, uint16_t x, uint16_t y,
    std::function<void(uint16_t, uint16_t, uint16_t, uint16_t)> eraseHandler)
    : Widget(x, y, 55, 15), model(model), eraseHandler(eraseHandler) {}
...
void ClockSmallWidget::update(uint32_t now) { redraw(); }
...
void ClockSmallWidget::redraw() {
  if (isVisible) {
    ...
    if (this->initialDraw || this->lastDrewMin != timeNow->tm_min) {
      ...
      boolean is24hStyle = model->isClock24hSytyle();
      uint8_t xmargin = is24hStyle ? 15 : 0;

      setTexFormat(ArialBold14, TL_DATUM, TFT_WHITE, TFT_TRANSPARENT, width);

      char time[8];
      sprintf(time, "%s%d", is24hStyle && timeNow->tm_hour < 10 ? "0" : "",
              is24hStyle ? timeNow->tm_hour : (timeNow->tm_hour + 11) % 12 + 1);
      this->xcolon = this->x + xmargin + 1 +
                     tft.drawString(time, this->x + xmargin, this->y);

      sprintf(time, "%02d%s", timeNow->tm_min,
              is24hStyle ? "" : (timeNow->tm_hour >= 12 ? "pm" : "am"));
    ...
    if (this->initialDraw || this->lastDrewSec != timeNow->tm_sec) {
      ...
      colon = !colon;
      setTexFormat(ArialBold14, TL_DATUM, colon ? TFT_WHITE : TFT_DARKGREY,
                   TFT_TRANSPARENT, 0);
