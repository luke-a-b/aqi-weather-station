...
CurrentWeatherWidget::CurrentWeatherWidget(CurrentWeatherModel *model, uint16_t y) : Widget(0, y, 240, 93),
                                                                                     model(model)
{
}
...
{    
    if (lastCurrentTemp != model->getCurrentWeatherTemp() || lastObservationTime != model->getObservationTime())
    {
        redraw();
    }
...
void CurrentWeatherWidget::redrawLocalTemp()
{
    ...
    {
        String degree = model->isMetricSelected() ? "°C" : "°F";
        uint8_t margin = x + width - 10;
        uint8_t temp_margin = margin - 35;

        setTexFormat(ARIAL_BOLD_14, BR_DATUM, TFT_ORANGE, TFT_BLACK, 0);
...
void CurrentWeatherWidget::redraw()
{
    if (isVisible)
    {
        ...
        tft.drawString(model->getCurrentWeatherDescription(), margin, y + 14 + 37 + 14 + 14 + 14);

        tft.setTextColor(TFT_WHITE, TFT_BLACK);
        tft.drawString(String(model->getCurrentWeatherPressure()) + "hPa", margin, y + 14 + 37 + 14 + 14);

        tft.setTextColor(TFT_SKYBLUE, TFT_BLACK);
        tft.drawString(String(model->getCurrentWeatherHumidity()) + "%", margin, y + 14 + 33);
        ...
        setTexFormat(ARIAL_BOLD_38, BR_DATUM, TFT_WHITE, TFT_TRANSPARENT, 0);
