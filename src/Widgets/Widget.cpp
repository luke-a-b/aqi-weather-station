...
// void Widget::setTexFormat(const String fontName, uint8_t datum, uint16_t fgcolor, uint16_t bgcolor, uint16_t padding)
// {
...
void Widget::setTexFormat(const uint8_t font[], uint8_t datum, uint16_t fgcolor, uint16_t bgcolor, uint16_t padding)
{
    tft.loadFont(font);
    if (bgcolor == TFT_TRANSPARENT)
    {
...
void Widget::setTexFormat(const GFXfont *font, uint8_t datum, uint16_t fgcolor, uint16_t bgcolor, uint16_t padding)
{
...
void Widget::setTexFormat(uint8_t datum, uint16_t fgcolor, uint16_t bgcolor, uint16_t padding)
{
...
void Widget::drawBmpFromFile(const String filename, uint8_t x, uint16_t y, TFT_eSprite *sprite)
{
    ...
    if (fileOk && read16(bmpFS) == 0x4D42)
    {
        ...
        if ((read16(bmpFS) == 1) && (read16(bmpFS) == 24) /* && (read32(bmpFS) == 0)*/)
        {
            ...
            bool oldSwapBytes = sprite == nullptr ? tft.getSwapBytes() : sprite->getSwapBytes() ;
            if (sprite == nullptr) tft.setSwapBytes(true);
            else sprite->setSwapBytes(true);
            ...
            if (sprite == nullptr) tft.setSwapBytes(oldSwapBytes);
            else sprite->setSwapBytes(oldSwapBytes);
            //Serial.printf(F("Bitmap %s loaded in %ul ms\n"), filename.c_str(), millis() - startTime);
        }
        else
        {
            fileOk = false;
            Serial.print(F("BMP format not recognized. Filename: "));
            Serial.println(filename);
        }
    }
