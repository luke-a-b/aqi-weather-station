...
//#define INDICATOR_COLOR 0x0011

//#define INDICATOR_COLOR TFT_BLACK
...
AqiPage::AqiPage(AqiDisplayModel *model, ClockModel *clock, CommandsHanler* commandsHandler) : Page(commandsHandler), model(model), clockSmallWidget(clock, 180, 2, &AqiPage::fillGradientRect), wifiStatusWidget(2, 0)
{
    resetValues();
...
void AqiPage::draw()
{
    ...
    if (this->initialDraw) fillGradientRect(0, 0, tft.width(), tft.height());
    
    if ((this->initialDraw || this->lastAqiLevel != model->getAqiLevel()) && !isnan(model->getAqiLevel())) // index
    {
    ...
    if (this->initialDraw || this->lastAqiPm25 != model->getAqiPm25()) // pm 2.5
    {
    ...
    y += offset;
    if (this->initialDraw || this->lastAqiPm10 != model->getAqiPm10()) // pm 10
    {
    ...
    y += offset;
    if (this->initialDraw || this->lastAqiTemp != model->getAqiTemp()) // temp
    {
        this->lastAqiTemp = model->getAqiTemp();
        printTextUnit(String(this->lastAqiTemp, 1), (model->isMetricSelected() ? "°C" : "°F"), 90, y);
    }

    y += offset;
    if (this->initialDraw || this->lastAqiHumidity != model->getAqiHumidity()) // humidity
    {
    ...
    y += offset;
    if (this->initialDraw || this->lastAqiPressure != model->getAqiPressure()) // pressure
    {
        this->lastAqiPressure = model->getAqiPressure();
        printTextUnit(String(model->getAqiPressure()), "hPa", 90, y);
    }

    if (this->initialDraw || this->aqiStationName != model->getAqiStaionName()) // station name
    {
        this->aqiStationName = model->getAqiStaionName();
        tft.setTextDatum(BC_DATUM);
        tft.loadFont(ARIAL_BOLD_14);
...
void AqiPage::printTextUnit(String text, String unit, uint16_t x, uint16_t y)
{
    if (!this->initialDraw) 
    {
        ...
        fillGradientRect(x, y, 95, 30);
    }
    Widget::setTexFormat(ARIAL_BOLD_28, BR_DATUM, TFT_WHITE, TFT_BLACK, 0);
    tft.setCursor(x, y);
    tft.print(text);
    tft.loadFont(ARIAL_BOLD_14);
...
void AqiPage::fillGradientRect(uint16_t x, uint16_t y, uint16_t width, uint16_t height)
{
...
void AqiPage::drawBitmap(uint16_t x, uint16_t y, const tImage *image)
{
    uint16_t x0, y0;
...
void AqiPage::fillArc(uint16_t x, uint16_t y, uint16_t r, float startAngle, float endAngle, uint32_t color)
{
