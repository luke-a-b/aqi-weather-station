...
PagesController::PagesController(Backlight *backlight, DisplayModel *model) : backlight(backlight),
                                                                              model(model),
                                                                              mainPage(model, this),
                                                                              aqiPage(model->getAqiDisplayModelAt(0), model, this),
                                                                              aboutPage(this)
{
...
void PagesController::showMainPage()
{
    showPage(&mainPage);
}
...
{
    while (currentAqiPage < MAX_AQI_STATIONS && model->getAqiDataModelAt(currentAqiPage)->stationUrl != "")
    {
        currentAqiPage++;
    }
...
{
    Serial.println(F("TODO: PagesController::showCurrentWeatherDetail"));
}

void PagesController::showForecastPage()
{
    Serial.println(F("TODO: PagesController::showForecastPage"));
}

void PagesController::showAboutPage()
{
    showPage(&aboutPage);
}
...
void PagesController::switchOffBacklight()
{
    backlight->setBacklight(false);
}

void PagesController::switchOnBacklight()
{
