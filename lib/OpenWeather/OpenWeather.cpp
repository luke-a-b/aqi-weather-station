...
***************************************************************************************/
bool OW_Weather::parseRequest(String url) {
  if (Secure) return parseRequestBearSSL(&url);
  else return parseRequestAXTLS(&url);
}

bool OW_Weather::parseRequestBearSSL(String* url) {
...
bool OW_Weather::parseRequestAXTLS(String* url) {
...
void OW_Weather::value(const char *val)
{
  ...
  if (currentParent == "") {
    ...
    if (currentKey == "timezone_offset") current->timezone_offset = (int32_t)value.toInt();
  }
